...
var Config = /** @class */ (function () {
    ...
    Config.prototype.getPlainObject = function () {
        ...
        function recursivelyExtend(key, job) {
            var _a, _b;
            if (job.extends) {
                var result = {};
                for (var _i = 0, _c = job.extends; _i < _c.length; _i++) {
                    var from = _c[_i];
                    var jobObj = ((_a = copy.jobs) === null || _a === void 0 ? void 0 : _a[from]) || ((_b = copy.jobs) === null || _b === void 0 ? void 0 : _b["." + from]);
                    if (!jobObj) {
                        console.warn("The job \"" + from + "\" does not exist, skipping...");
                        continue;
                    }
                    result = deepmerge_1.default(result, recursivelyExtend(key, jobObj));
                }
                return deepmerge_1.default(result, job);
            }
            return job;
        }
        // Resolve `extends`
        ...
        for (var _a = 0, jobIds_2 = jobIds; _a < jobIds_2.length; _a++) {
            var key = jobIds_2[_a];
            var job = copy.jobs[key];
            if (job.extends) {
